...
export async function runProbeScenarios({transport,emit=()=>{}}) {
  ...
  async function check(id,expected,fn) {
    let result;try{const detail=await fn();result={id,expected,passed:true,status:'passed',actual:detail??'Erwartete Wirkung nach erneutem Lesen bestätigt.'};}
      ...
      result={id,expected,passed:false,status:code==='unsupported'?'unsupported':'failed',actual:code,...(Number.isInteger(error?.status)?{httpStatus:error.status}:{})};}
  ...
  async function fixture(name){const file=await transport.create({parentId:folder.id,name,value:initial()});return transport.read(file.id);}
  ...
  await check('invalid-token','Falsches If-Match ergibt 412; Inhalt bleibt unverändert.',async()=>{
    const before=await fixture('invalid-token');await rejects(()=>transport.updateIfUnchanged({...before,etag:'"deliberately-invalid-probe-token"'},{...before.value,sequence:99}),'stale');assert(same((await transport.read(before.id)).value,before.value));
  });
  await check('stale-write','Verbrauchtes If-Match ergibt 412; erster Inhalt bleibt.',async()=>{
    ...
    await rejects(()=>transport.updateIfUnchanged(before,{...before.value,sequence:2}),'stale');assert(same((await transport.read(before.id)).value,winner));
  });
  await check('initialization','Zwei Initialisierer: genau eine verbindliche Ordnerbindung.',async()=>{
    ...
    const winner=results[0].status==='fulfilled'?a:b;assert((await transport.read(folder.id)).properties.coordinator===winner.id);
  });
  await check('two-purchases','Zwei Käufe zu 800 bei 1000 Guthaben: genau einer, Rest 200.',async()=>{
    ...
    assert(results.filter(r=>r.status==='fulfilled').length===1);assert(results.filter(r=>r.status==='rejected'&&r.reason.code==='stale').length===1);
    const current=(await transport.read(before.id)).value;assert(current.spent===800&&current.operations.length===1);
  ...
  await check('response-loss','Absichtlich verworfene Erfolgsantwort: Beleg mit gleicher ID gefunden, keine zweite Ausgabe.',async()=>{
    ...
    let current=await transport.read(before.id);assert(current.value.operations.some(op=>op.id===request.id));
    await transport.updateIfUnchanged(current,purchase(current.value,{id:'later-operation',article:'b',price:100}));
    current=await transport.read(before.id);
    ...
    assert((await transport.read(before.id)).value.spent===900);
  ...
  await check('duplicate-operation','Vorgang und vorhandener Besitz erzeugen keine zweite Ausgabe.',async()=>{
    ...
    const current=(await transport.read(before.id)).value;assert(same(purchase(current,request),current));assert(same(purchase(current,{...request,id:'different'}),current));
  ...
  await check('reset-race','Kauf und Epochenwechsel teilen dieselbe Schreibgrenze; beide Gewinnerreihenfolgen.',async()=>{
    for(const resetFirst of [true,false]){
      ...
      await transport.updateIfUnchanged(before,winner);await rejects(()=>transport.updateIfUnchanged(before,loser),'stale');assert(same((await transport.read(before.id)).value,winner));
  ...
  await check('create-conflict','Erneutes Create prüft identischen Inhalt; abweichender Inhalt wird abgewiesen.',async()=>{
    ...
    await rejects(()=>transport.retryCreate(before.id,{...before.value,earned:2000}),'collision');assert(same((await transport.read(before.id)).value,before.value));
  });

...
function buttons(){el('connect').disabled=busy||connected;el('disconnect').disabled=busy||!connected;el('start').disabled=busy||!connected||!el('consent').checked;el('source').disabled=busy;el('scope').disabled=busy;el('consent').disabled=busy;el('download').disabled=!report||busy;}
...
el('start').addEventListener('click',async()=>{
  ...
  if(['read-stability','metadata-coordination','immutable-purchases'].includes(probeScope)&&etagSource!=='v2-coherent'){
    const scopeName=probeScope==='immutable-purchases'?'Kaufablauf mit Belegkette prüfen':probeScope==='metadata-coordination'?'Ordner-Koordination gezielt prüfen':'Dateilesen gezielt untersuchen';
  ...
  try{
    const result=await runProbeScenarios({transport:createProbeTransport({token:()=>session.getToken(),etagSource}),probeScope,emit:check=>{
    ...
    const apiPaths=probeScope==='metadata-coordination'
      ...
      :probeScope==='immutable-purchases'
      ?{idReservation:'GET /drive/v3/files/generateIds',create:'POST /drive/v3/files oder /upload/drive/v3/files',metadataRead:'2 × GET /drive/v2/files/{ownedFolderId}?fields=...version,etag,...; cache=no-store',immutableRead:'2 × GET /drive/v2/files/{ownedReceiptId}?fields=... plus GET alt=media; cache=no-store',metadataUpdate:'PUT /drive/v2/files/{ownedFolderId}',condition:'If-Match'}
    ...
    report={kind:'synthetic-shop-probe',diagnosticVersion:10,startedAt,completedAt:new Date().toISOString(),origin:location.origin,userAgent:navigator.userAgent,etagSource,probeScope,apiPaths,...result};
    el('status').textContent=probeScope==='immutable-purchases'
      ?result.passed?'Die Belegkettenprobe ist vollständig bestanden. Sie gibt keine Produkt- oder Zwei-Geräte-Garantie; der Produktshop bleibt gesperrt.':'Die Belegkettenprobe ist fehlgeschlagen oder unvollständig. Sie gibt keine Produkt- oder Zwei-Geräte-Garantie; der Produktshop bleibt gesperrt.'
      :probeScope==='metadata-coordination'
      ...
      :result.passed?'Alle isolierten Probeszenarien bestanden. Produktshop bleibt gesperrt; weitere Nachweise stehen aus.':'Kaufkoordination nicht ausreichend nachgewiesen. Produktshop bleibt gesperrt.';
...
el('download').addEventListener('click',()=>{
  ...
  const link=document.createElement('a');link.href=url;link.download='shop-probe-bericht10.json';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);
});
buttons();

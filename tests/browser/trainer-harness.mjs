...
export async function createTrainerHarness({basePath = ''} = {}) {
  ...
  server.on('request', (request, response) => {
    ...
    if (pathname === `${basePath}/trainer/sw.js` && workerVersion !== 'v9') {
      let source = productWorker.replace(
        'const CACHE_NAME = `${CACHE_OWNER}v9`;',
  ...
  return {
    ...
    setServiceWorkerVersion(version, {activationDelayMs = 0} = {}) {
      if (!/^v[1-9][0-9]+$/u.test(version)) throw new TypeError('Synthetic worker version must be v10 or later.');

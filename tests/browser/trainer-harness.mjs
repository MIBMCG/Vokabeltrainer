...
export async function createTrainerHarness({basePath = ''} = {}) {
  ...
  server.on('request', (request, response) => {
    ...
    if (pathname === `${basePath}/trainer/sw.js` && workerVersion !== 'v16') {
      let source = productWorker.replace(
        'const CACHE_NAME = `${CACHE_OWNER}v16`;',
  ...
  return {
    ...
    setServiceWorkerVersion(version, {activationDelayMs = 0} = {}) {
      if (!/^v(?:1[7-9]|[2-9][0-9]|[1-9][0-9]{2,})$/u.test(version)) {
        throw new TypeError('Synthetic worker version must be v17 or later.');
      }

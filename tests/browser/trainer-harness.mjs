...
export async function createTrainerHarness({basePath = ''} = {}) {
  ...
  server.on('request', (request, response) => {
    ...
    if (pathname === `${basePath}/trainer/sw.js` && workerVersion !== 'v10') {
      let source = productWorker.replace(
        'const CACHE_NAME = `${CACHE_OWNER}v10`;',
  ...
  return {
    ...
    setServiceWorkerVersion(version, {activationDelayMs = 0} = {}) {
      if (!/^v(?:1[1-9]|[2-9][0-9]|[1-9][0-9]{2,})$/u.test(version)) {
        throw new TypeError('Synthetic worker version must be v11 or later.');
      }

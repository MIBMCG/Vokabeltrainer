...
test('mixed backup dependencies preserve support-only policies and resets without activating them',async()=>{
  ...
  const restored=backupLedger(await validateBackup(result));
  assert.deepEqual(currentPolicy(restored,'p1'),{eventId:null,policy:DEFAULT_POLICY});
  assert.equal(currentGenerations(restored,'p1').find(e=>e.wordId==='w1').generationId,null);
  assert.equal(projectSchedule({ledger:restored,profileId:'p1',policy:DEFAULT_POLICY,day:'2026-09-18'}).words.get('w1').get('learn-w1').streak,0);
  const chooseRules=await restore.previewAdoption(['rules','reset']);
  await restore.adopt({eventIds:['rules','reset'],previewId:chooseRules.previewId});
  const selected=commands.getState().ledger;
  assert.deepEqual(currentPolicy(selected,'p1'),{eventId:'rules',policy});
  assert.equal(currentGenerations(selected,'p1').find(e=>e.wordId==='w1').generationId,'reset');
  assert.equal(projectSchedule({ledger:selected,profileId:'p1',policy,day:'2026-09-18'}).words.get('w1').get('learn-w1').streak,1);
  assert.equal(project(selected).profiles.p1.points,10);

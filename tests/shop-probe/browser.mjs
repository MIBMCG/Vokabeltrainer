...
for(const {basePath,noEtag=false,source='media',noJsonEtag=false,ignore=false,ignoreMediaCondition=false,ignoreMetadataCondition=false} of cases)test(`isolated shop probe UI with synthetic Google boundary ${basePath||'root'} ${source}${noEtag?' missing headers':''}${noJsonEtag?' missing JSON ETag':''}${ignore||ignoreMediaCondition||ignoreMetadataCondition?' ignored conditions':''}`,async()=>{
  ...
  try{
    ...
    const fixture=source==='v2-coherent'
      ?v2CoherentDriveFixture({ignoreMediaCondition,ignoreMetadataCondition})
    ...
    const expectedPass=!ignore&&!ignoreMediaCondition&&!ignoreMetadataCondition&&!noJsonEtag&&(!noEtag||source==='v2-json');
    ...
    assert.equal(report.diagnosticVersion,5);assert.equal(report.etagSource,source);
    if(source==='v2-json'){
    ...
    if(source==='v2-coherent'){
      ...
      assert.equal(report.apiPaths.metadataRead,'GET /drive/v2/files/{ownedId}?fields=...version,etag');
    ...
    const screenshotName=ignore?'v5-write-diagnostics':ignoreMediaCondition?'v2-coherent-media-ignore':ignoreMetadataCondition?'v2-coherent-metadata-ignore':noJsonEtag?'v2-json-missing-token':source==='v2-coherent'?'v2-coherent':source==='v2-json'?'v2-json':noEtag?'missing-headers':basePath?'subpath':'root';

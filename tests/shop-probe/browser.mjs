...
for(const {basePath,noEtag=false,source='media',noJsonEtag=false} of [{basePath:'',noEtag:false},{basePath:'/isolated',noEtag:false},{basePath:'',noEtag:true},{basePath:'',noEtag:true,source:'v2-json'},{basePath:'',noEtag:true,source:'v2-json',noJsonEtag:true}])test(`isolated shop probe UI with synthetic Google boundary ${basePath||'root'} ${source}${noEtag?' missing headers':''}${noJsonEtag?' missing JSON ETag':''}`,async()=>{
  ...
  try{
    ...
    const fixture=driveFixture({noEtag,noJsonEtag});let loginCalls=0;
    ...
    const expectedPass=!noJsonEtag&&(!noEtag||source==='v2-json');
    ...
    assert.equal(downloaded.suggestedFilename(),'shop-probe-bericht3.json');
    assert.equal(report.diagnosticVersion,3);assert.equal(report.etagSource,source);
    if(source==='v2-json'){
    ...
    const screenshotName=noJsonEtag?'v2-json-missing-token':source==='v2-json'?'v2-json':noEtag?'missing-headers':basePath?'subpath':'root';

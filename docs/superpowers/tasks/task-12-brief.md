...
## Agreed safe-update boundary (decision 24)
Implement shell.pauseForUpdate as pauseAndSave. Non-whitespace unsubmitted input rejects activation with typed-answer-present and a clear instruction to submit or deliberately clear first; never clear/score automatically. Pending practice actions reject retryably/not-ready using the actual action-busy state, not an assumed DOM attribute. Empty asking or fully committed feedback/round can reload and resume unchanged; no next/finish/abandon/bonus. No schema or new draft storage. Offer executable updates only at safe views or guard against active setup/adult forms/saves as well. Readonly practice guard helper and shell integration are authorized task files. Test these actual UI boundaries and controllerchange-driven reload. The existing committed round is the durable checkpoint; unsaved input explicitly prevents reaching it.

## Review-driven activation boundary (decision 25)

After its safe checks shell.pauseForUpdate locks editing/navigation atomically and returns an idempotent release callback. The page requests activation through the active worker, which verifies its actually controlled client set before forwarding to the waiting worker; the waiting worker accepts only that active worker as sender. Correlate requests and replies and validate the expected sender. Rejection, post failure, timeout and destruction release the UI lock and clean listeners/timers. Once released, a delayed controllerchange must not blindly reload newly entered data. Cover the real delayed worker transition and the failure/late-transition path. No new persisted schema or round events; preserve the existing explicit update choice.

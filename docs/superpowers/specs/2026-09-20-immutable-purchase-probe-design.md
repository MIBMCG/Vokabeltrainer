...
Der Versuch legt einen synthetischen Hauptordner und je Szenario genau einen gemeinsamen Unterordner an. Beide logischen Clients erhalten dieselbe bereits geprüfte Ordner-ID. Keine Suche nach Namen, keine getrennt erzeugten Konten mit anschließendem „Gewinner nach Zeitstempel“.
...
3. `readImmutable(ref)` prüft registrierte ID/Hash, Elternordner, Titel, MIME-Typ, App/Laufbindung und Papierkorbstatus vor und nach dem Inhaltsabruf; alle Aufrufe ohne HTTP-Cache. Nur ein inhaltlich passender SHA-256-Wert liefert JSON zurück.
...
`createPurchaseCoordinator({transport,anchorId})` bietet `read()`, `prepare(operation)`, `commit(ticket)` und `recover(operation)`.
...
1. Zwei Initialisierungen bereiten sich am selben leeren Anker vor; genau ein PUT mit 2xx, genau ein 412, genau eine geprüfte Wurzel. Erhaltungsmarker bleibt erhalten.
2. Zwei Käufe zu je 800 aus 1000 Punkten: beide vorbereiten, parallel schreiben. Genau ein 2xx, ein 412; genau ein Besitz und 200 Restpunkte. Der verlorene Kandidat zählt nicht.
...
Exporte enthalten nur feste Szenarionamen, Ergebnis-/Fehlerklassen, begrenzte Zähler, HTTP-Status und Boolesche Vergleiche. Keine Datei-IDs, ETags, Hashwerte, Inhalte, Google-Tokens, Operationsobjekte oder rohe Fehlermeldungen. Beide logischen Clients laufen in einem Browser. Alle neuen Objekte bleiben synthetisch und erhalten.
